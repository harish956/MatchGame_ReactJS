...
const TabItem = props => {
  ...
  return (
    <li className="tab-item-container ">
      <button
        type="button"
        className={`tabItem ${activeTabBtnClassName}`}
...
const ProjectItem = props => {
  const {projectDetails, fun} = props
  ...
  return (
    <li className="project-item-container">
      <button type="button" onClick={checking} className="project-item-button">
        <img
          src={thumbnailUrl}
          alt="thumbnail"
...
class ImagesListProvider extends Component {
  state = {
  ...
  imageGenerator = () => {
    const ind = Math.floor(Math.random() * (30 - 0) + 0)
    this.setState(() => ({image: ind}))
  }
  ...
  incrementTimeElapsedInSeconds = () => {
    ...
    if (time === 0) {
      ...
      this.setState(() => ({time: 0, isTimerRunning: false}))
    } else {
  ...
  fun = thumbnailUrl => {
    ...
    if (thumbnailUrl === imagesList[image].thumbnailUrl) {
  ...
  gamePage = () => {
    const {image, activeTabId} = this.state
    const filteredProjects = this.getFilteredProjects()
    return (
      <div className="imagesDisplayContainer">
        <img
          src={imagesList[image].imageUrl}
          alt="match"
    ...
    return (
      <div className="main-background-container">
        ...
        <div className="body-container">
          {isTimerRunning ? this.gamePage() : this.scoreCard()}
        </div>
      </div>
    )
  }
